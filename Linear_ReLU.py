...
print(len(test_dataloader))  # there are 157 batches 
print(len(test_dataloader.dataset)) # there are 10000 images in the test dataset. 
...
def test_loop(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader: # every iteration is looking at one batch of 64 images. 
            X, y = X.to(device), y.to(device)
            pred = model(X)
...
# torch.save(model.state_dict(), "model_weights_linear_ReLU.pth")
